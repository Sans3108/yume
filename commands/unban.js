...
module.exports = {
    ...
    execute(message, args, bot, color) {
        ...
        .setDescription(`\`\`${message.author.tag}\`\` unbanned \`\`${bannedMember.tag}\`\`.`);
        modch.send(modemb);
        ...
        bot.fetchUser(args[0])
            .then(bannedMember => {
                let embed3 = new Discord.RichEmbed()
                    .setColor(color)
                    .setDescription(`${bannedMember.tag} has been unbanned by ${message.author}`);

                message.guild.fetchBans().then(banList => {
                    let bannedUser = banList.find(user => user.id === bannedMember.id);

                    if (bannedUser) {
                        message.guild.unban(bannedMember).then(message.channel.send(embed3));
                    } else {
